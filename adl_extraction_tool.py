...
def transform_definition_to_JSON(definition_text):
	...
	#Remember where we have series of variables (from C_DV_QUANTITY sections)
	pattern = re.compile(r'("\n[\t]*")')
	matches = pattern.finditer(definition_text)
	if matches:
		for match in matches:
	...
	definition_text = '\n'.join(lines)
	definition_text = definition_text.replace(']": "*"', ']": {"value": "*"}')
	...
	for file in fileList:
		...
		if 'other_details' in description_JSON.keys():
			if isinstance(description_JSON['other_details'], dict):
				...
				if 'revision' in description_JSON['other_details'].keys():
					node['revision'] = str(description_JSON['other_details']['revision'])
